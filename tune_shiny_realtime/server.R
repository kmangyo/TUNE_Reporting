...
shinyServer(function(input, output, session) {
  ...
  output$report_time <- DT::renderDataTable(DT::datatable({
    if(input$dl_url==c('')){
      report<-data.frame()
    } else {
    ...
    report<- report %>% arrange(id)
    report
    }
  }))

  output$report_cumsum <- DT::renderDataTable(DT::datatable({
    if(input$dl_url==c('')){
      report<-data.frame()
    } else {
    report<-read.csv(input$dl_url)
    ...
    report<-report[c('Timestamp','Publisher.name','Advertiser_sub_ad.name','cumsum_ad_click','cumsum_install','cumsum.CVR.install','cumsum_revenues','cumsum_PURCHASE','cumsum_REGISTRATION','cumsum_OPEN','id')]
    report<- report %>% arrange(id)
    report
    }
  }))
