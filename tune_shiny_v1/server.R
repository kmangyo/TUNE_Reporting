#https://kmangyo.shinyapps.io/tune-reporting_v2/
...
shinyServer(function(input, output, session) {
  ...
  ad.name <- toupper((levels(report[,c(11)])))
  ad.name
  }
  })

  publisher.names <- reactive({
  if (input$dl_url==c('')){
      report<-c('')
      } else {
  report <- read.csv(input$dl_url)        
  publisher.names <- toupper(levels(report[,c(3)]))
  publisher.names
  }
  })
  ...
  output$report <- DT::renderDataTable(DT::datatable({
    ...
    report$Advertiser_sub_ad.name <- toupper(report$Advertiser_sub_ad.name)
    report$Publisher.name <- toupper(report$Publisher.name)
    report <- filter(report, Publisher.name %in% input$file1)
    report <- filter(report, Advertiser_sub_ad.name %in% input$file2)
    report<-report[c(1:5,10,6:9)]
    names(report)<-c('Time','Publisher','sub_ad.name','Ad_click','Install','CVR.istl','Rev.','Pur.','Reg.','Open')
    report
    }
  }))

  output$report_off <- DT::renderDataTable(DT::datatable({
    if(input$dl_url==c('')){
      report<-data.frame()
    } else {
    report <- read.csv(input$dl_url)
    ...
    report <- report[c('timestamp','publisher.name','advertiser_sub_ad.name','ad_click.','installs.INSTALL','revenues.PURCHASE','events.PURCHASE','events.REGISTRATION','opens.OPEN')]
    report <- subset(report, publisher.name!=c('organic'))
    names(report) <- c('Timestamp','Publisher.name','Advertiser_sub_ad.name','Ad_click','Install','Revenues','Purchase','Registration','Open')
    report$Advertiser_sub_ad.name<-toupper(report$Advertiser_sub_ad.name)
    report$Publisher.name<-toupper(report$Publisher.name)
    if(is.null(input$file1)|is.null(input$file2)){
      report$CVR.install <- with(report, (Install/Ad_click))
      report$CVR.install[is.na(report$CVR.install)] <- 0
      report$CVR.install[is.infinite(report$CVR.install)] <- 0
      report$CVR.install <- percent(report$CVR.install)
      names(report) <- c('Timestamp','Publisher.name','Advertiser_sub_ad.name','Ad_click','Install','Revenues','Purchase','Registration','Open','CVR.install')
      ...
      report$Publisher.name <- toupper(report$Publisher.name)
      report<-report[c(1:5,10,6:9)]
      names(report)<-c('Time','Publisher','sub_ad.name','Ad_click','Install','CVR.istl','Pur.','Rev.','Reg.','Open')
      report
      } else {
    report <- filter(report, Publisher.name %in% input$file1)
    report <- filter(report, !(Advertiser_sub_ad.name %in% input$file2))
    report <- report %>% group_by(Timestamp, Publisher.name) %>% summarise(Ad_click=sum(Ad_click),Install=sum(Install),Revenues=sum(Revenues),Purchase=sum(Purchase),Registration=sum(Registration),Open=sum(Open))
    report$CVR.install <- with(report, (Install/Ad_click))
    report$CVR.install[is.na(report$CVR.install)] <- 0
    report$CVR.install[is.infinite(report$CVR.install)] <- 0
    report$CVR.install <- percent(report$CVR.install)  
    report <- report[c(1:4,9,5:8)]
    names(report)<-c('Time','Publisher','Ad_click','Install','CVR.istl','Rev.','Pur.','Reg.','Open')
    report
    }
    }
  }))
  ...
  output$report_puv <- DT::renderDataTable(DT::datatable({
    ...
    report_puv$advertiser_sub_ad.name<-toupper(report_puv$advertiser_sub_ad.name)
    report_puv$publisher.name<-toupper(report_puv$publisher.name)
    report_puv <- filter(report_puv, publisher.name %in% input$file1)
    report_puv <- filter(report_puv, advertiser_sub_ad.name %in% input$file2)
    names(report_puv)<-c('Time','Publisher','sub_ad.name','PUV')
    report_puv
    }
  }))

  output$report_puv_off <- DT::renderDataTable(DT::datatable({
    if(input$dl_url_puv==c('')){
      report_puv <- data.frame()
    } else {
    report_puv <- read.csv(input$dl_url_puv)
    ...
    if(is.null(input$file1)|is.null(input$file2)){
      ...
      report_puv <- filter(report_puv, publisher.name %in% input$file1)
      report_puv <- filter(report_puv, !(advertiser_sub_ad.name %in% input$file2))
      report_puv <- report_puv %>% group_by(created, publisher.name) %>% summarise(count=sum(count))
      names(report_puv)<-c('Time','Publisher','PUV')
      report_puv
    }
    }
  }))
